...
function pickToken(req) {
  return req.header("x-fb-access-token") || req.user?.meta?.accessToken || "";
}
...
async function requireMetaToken(req, res, next) {
  // Force-select the token field which is default:select:false.
  if (!req.user.meta?.accessToken) {
...
router.post("/disconnect", async (req, res, next) => {
  try {
    await User.findByIdAndUpdate(req.user._id, {
      $set: { meta: { accessToken: "", fbUserId: "", fbUserName: "", adAccountId: "", accounts: [], connectedAt: null } },
    });
...
router.get("/status", async (req, res, next) => {
  try {
    const u = await User.findById(req.user._id).select("+meta.accessToken");
    const connected = !!u?.meta?.accessToken;
    res.json({
      connected,
      fbUser: connected ? { id: u.meta.fbUserId, name: u.meta.fbUserName } : null,
      accounts: connected ? u.meta.accounts : [],
      selectedAdAccountId: connected ? u.meta.adAccountId : "",
      connectedAt: connected ? u.meta.connectedAt : null,
    });
  } catch (err) { next(err); }
});
