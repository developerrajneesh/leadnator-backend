...
async function loadConnection(userId) {
  return WhatsAppConnection.findOne({ user: userId }).select("+accessToken +webhookVerifyToken");
}
...
router.get("/webhook", async (req, res, next) => {
  try {
    let conn = await WhatsAppConnection.findOne({ user: req.user._id }).select("+webhookVerifyToken");
...
router.post("/webhook/rotate-token", async (req, res, next) => {
  try {
    const conn = await WhatsAppConnection.findOne({ user: req.user._id }).select("+webhookVerifyToken");
...
router.put("/webhook/verify-token", async (req, res, next) => {
  try {
    ...
    const conn = await WhatsAppConnection.findOneAndUpdate(
      { user: req.user._id }, { $set: { webhookVerifyToken: token } }, { new: true }
    );
    if (!conn) return res.status(404).json({ error: "Connect WhatsApp first." });
    res.json({ ok: true });
  } catch (err) { next(err); }
...
router.get("/embedded-config", (_req, res) => {
  res.json({
    fbAppId:   process.env.WHATSAPP_FB_APP_ID || process.env.FB_APP_ID || "",
    configId:  process.env.WHATSAPP_FB_CONFIG_ID || process.env.FB_CONFIG_ID || "",
...
router.post("/embedded-connect", async (req, res, next) => {
  try {
    ...
    res.json({ connected: true, connection: conn.toJSON(), businessId });
  } catch (err) { next(err); }
});
...
router.post("/conversations/:phone/read", async (req, res, next) => {
  try {
    await WhatsAppContact.updateOne(
      { user: req.user._id, phone: req.params.phone },
      {
        $setOnInsert: { user: req.user._id, phone: req.params.phone, name: req.params.phone },
...
router.delete("/conversations/:phone", async (req, res, next) => {
  try {
    ...
    const msgs = await WhatsAppMessage.deleteMany({ user: req.user._id, contactPhone: phone });
    const contact = await WhatsAppContact.deleteOne({ user: req.user._id, phone });
...
router.post("/send-template", requireWa, async (req, res, next) => {
  try {
    ...
    const msg = await WhatsAppMessage.create({
      user: req.user._id, contactPhone: to, direction: "outbound",
  ...
  try {
    ...
    for (const c of contacts) {
      try {
        ...
        await WhatsAppMessage.create({
          user: req.user._id, contactPhone: c.phone, direction: "outbound",
...
router.get("/conversations", async (req, res, next) => {
  try {
    ...
    const contacts = await WhatsAppContact
      .find({ user: req.user._id, phone: { $in: phones } })
      .populate("labels");
    ...
    const unreadAgg = await WhatsAppMessage.aggregate([
      { $match: { user: req.user._id, direction: "inbound", contactPhone: { $in: phones } } },
      {
        $group: {
          _id: "$contactPhone",
          newest: { $max: "$ts" },
...
router.get("/conversations/:phone", async (req, res, next) => {
  try {
    const messages = await WhatsAppMessage
      .find({ user: req.user._id, contactPhone: req.params.phone })
      .sort({ ts: 1 });
    const contact = await WhatsAppContact
      .findOne({ user: req.user._id, phone: req.params.phone })
      .populate("labels");
...
router.post("/conversations/:phone/reply", requireWa, async (req, res, next) => {
  try {
    ...
    const messageId = result?.messages?.[0]?.id || "";
    const msg = await WhatsAppMessage.create({
      user: req.user._id, contactPhone: req.params.phone, direction: "outbound",
      type: "text", text: body, messageId, status: "sent",
    });
    emitToUser(req.user._id, "wa.outbound", { message: msg.toJSON() });
...
router.post("/conversations/:phone/reply-media", requireWa, upload.single("file"), async (req, res, next) => {
  try {
    ...
    const msg = await WhatsAppMessage.create({
      user: req.user._id, contactPhone: phone, direction: "outbound",
      type: kind,
      text: caption,
      messageId, status: "sent",
...
router.get("/flows", async (req, res, next) => {
  try {
    const flows = await WhatsAppFlow.find({ user: req.user._id }).sort({ createdAt: -1 });
    res.json({ flows });
  } catch (err) { next(err); }
});
...
router.get("/flows/:id", async (req, res, next) => {
  try {
    const flow = await WhatsAppFlow.findOne({ _id: req.params.id, user: req.user._id });
...
router.post("/flows", async (req, res, next) => {
  try {
    ...
    if (!name) return res.status(400).json({ error: "name required" });
    const flow = await WhatsAppFlow.create({ user: req.user._id, name, nodes, edges, status });
...
router.delete("/flows/:id", async (req, res, next) => {
  try {
    const r = await WhatsAppFlow.deleteOne({ _id: req.params.id, user: req.user._id });
