...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    log("→ 403: no matching verify token in DB. Did you paste the token from /whatsapp/webhook into Meta dashboard?");
  } catch (err) {
...
router.post("/", async (req, res) => {
  ...
  log(`POST event received at ${new Date().toISOString()}`);
  dumpPayload(req.body);

  try {
    const entry = req.body?.entry?.[0];
    if (!entry) {
      log("⚠ no entry[0] — ignoring");
      remember({ kind: "ignored", reason: "no entry[0]", body: req.body });
      return;
    }
    ...
    if (!value) {
      log("⚠ no changes[0].value — ignoring");
      remember({ kind: "ignored", reason: "no changes.value", body: req.body });
      return;
    }

    const phoneNumberId = value?.metadata?.phone_number_id;
    log(`phone_number_id = ${phoneNumberId}`);
    if (!phoneNumberId) {
      log("⚠ no metadata.phone_number_id — cannot route to a user");
      remember({ kind: "ignored", reason: "no phone_number_id", body: req.body });
      return;
    }

    const conn = await WhatsAppConnection.findOne({ phoneNumberId }).select("+accessToken");
    if (!conn) {
      log(`⚠ no WhatsAppConnection for phoneNumberId ${phoneNumberId}. Connect this number in /whatsapp/settings.`);
      remember({ kind: "ignored", reason: "no connection", phoneNumberId });
      return;
    }
    log(`→ owner: user ${conn.user}`);
    ...
    for (const st of value.statuses || []) {
      ...
      await WhatsAppMessage.updateOne(
        { user: conn.user, messageId: st.id },
    ...
    for (const msg of value.messages || []) {
      ...
      try {
        const ins = await WhatsAppContact.updateOne(
          { user: conn.user, phone: fromPhone },
          { $setOnInsert: { user: conn.user, phone: fromPhone, name: profileName || fromPhone } },
          { upsert: true }
        );
      ...
      try {
        const settings = await LeadSettings.forUser(conn.user);
    ...
    if (!(value.messages || value.statuses)) {
      log("⚠ payload had neither messages[] nor statuses[] — probably an unsupported field. Check Meta subscription fields.");
      remember({ kind: "unsupported", body: req.body });
    }
  } catch (err) {
...
async function tryChatbotReply({ conn, fromPhone, text, buttonId = "" }) {
  ...
  for (const payload of payloads) {
    ...
    const botMsg = await WhatsAppMessage.create({
      user: conn.user, contactPhone: fromPhone, direction: "outbound",
      type: payload.type,
