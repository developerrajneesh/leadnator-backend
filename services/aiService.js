...
${brief.cta || "Start free →"} ${brief.link || "leadnator.com"}`,
...
function resolveProvider() {
  ...
  if (explicit === "openai") return openaiKey ? "openai" : "template";
  if (explicit === "gemini") return geminiKey ? "gemini" : "template";
  // No AI_PROVIDER — prefer OpenAI when configured, else Gemini
...
async function generateText({ prompt, system, model, temperature } = {}) {
  ...
  if (provider === "template") {
    throw new Error(
      "AI is not configured. Set OPENAI_API_KEY and AI_PROVIDER=openai in backend/.env, then restart the server."
    );
  }
  if (provider === "gemini") {
